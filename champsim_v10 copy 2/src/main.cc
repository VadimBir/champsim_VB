        ...
        for (int i=0; i<NUM_CPUS; i++) {
            ...
            if (show_heartbeat && (ooo_cpu[i].num_retired >= ooo_cpu[i].next_print_instruction)) {
                ...
                cout << " " << STR_IPC_NOW << FIXED_FLOAT2(heartbeat_ipc)
                     ...
                     << left 
                     << " " STR_APC
                     << setw(3) << right <<  FIXED_FLOAT2(lpm[i][L1D_type].apc_accessesDivActiveMemCy_ratio) << " "
                     << setw(3) << right <<  FIXED_FLOAT2(lpm[i][L2C_type].apc_accessesDivActiveMemCy_ratio) << " "
                     << setw(3) << right <<  FIXED_FLOAT2(lpm[i][LLC_type].apc_accessesDivActiveMemCy_ratio) << " "
                     << setw(3) << right <<  FIXED_FLOAT2(get_recursive_apc(i))
                     << left
                     << " " STR_LPM
                     << setw(3) << right <<  FIXED_FLOAT2(lpm[i][L1D_type].lpmr_activeMemCyDivIdealCy_ratio) << " "
                     << setw(3) << right <<  FIXED_FLOAT2(lpm[i][L2C_type].lpmr_activeMemCyDivIdealCy_ratio) << " "
                     << setw(3) << right <<  FIXED_FLOAT2(lpm[i][LLC_type].lpmr_activeMemCyDivIdealCy_ratio) << " "
                     << setw(3) << right <<  FIXED_FLOAT2(get_LPMR_global_std(i))
                     << left
                     << " " STR_cAMT
                     << setw(5) << right <<  FIXED_FLOAT2(lpm[i][L1D_type].camat_activeMemCyDivAccesses_ratio) << " "
                     << setw(5) << right <<  FIXED_FLOAT2(lpm[i][L2C_type].camat_activeMemCyDivAccesses_ratio) << " "
                     << setw(5) << right <<  FIXED_FLOAT2(lpm[i][LLC_type].camat_activeMemCyDivAccesses_ratio) << " "
                     << setw(5) << right <<  FIXED_FLOAT2(get_recursive_camat(i))
                     << left 
