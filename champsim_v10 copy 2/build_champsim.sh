...
if [ "$#" -ne 5 ]; then
    ...
    echo "Usage: ./build_champsim.sh [core_arch] [l1d_pref] [l2c_pref] [llc_pref] [num_core]"
    exit 1
fi
...
re='^[0-9]+$'
if ! [[ $NUM_CORE =~ $re ]] ; then
    echo "[ERROR]: num_core is NOT a number <${NUM_CORE}>" >&2;
    exit 1
fi
...
if [ "$NUM_CORE" -gt "0" ]; then
    sed -i.bak 's/#define NUM_CPUS [0-9]*/#define NUM_CPUS '${NUM_CORE}'/' inc/champsim.h
    echo "Building multi-core ChampSim... Number of core: ${NUM_CORE} ... Core architecture ${CORE_UARCH}"
    #sed -i.bak 's/#define NUM_CPUS [0-9]*/#define NUM_CPUS '${NUM_CORE}'/' inc/champsim.h
...
#	sed -i.bak 's/\<DRAM_CHANNELS_LOG2 0\>/DRAM_CHANNELS_LOG2 1/g' inc/champsim.h
    ...
    if [ "$NUM_CORE" -lt "1" ]; then
        ...
        sed -i "s/^#define NUM_CPUS [0-9]\+/#define NUM_CPUS ${NUM_CORES}/"  ./$champsimDirName/inc/champsim.h
    fi
fi
echo
...
#sed -i  "s/#define NUM_CPUS [0-9]\+/#define NUM_CPUS $NUM_CPUS/" ./inc/champsim.h
...
echo "NUM_CPUS value: $NUM_CORE // if issue fix build_champsim.sh"
...
# # grep the changed line
# echo "NUM_CPUS value after change"
# grep -i "NUM_CPUS" ./inc/champsim.h
