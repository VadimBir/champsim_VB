    ...
    const uint64_t* p_minor_fault)
{
    ...
    // ── INSERT cache_stats (per cpu × per cache level) ──────────────────
    {
        ...
        auto insert_cache = [&](int cpu, CACHE* cache, const char* name) {
            ...
            // lpm metrics
            sqlite3_bind_double(stmt, c++, lpm[cpu][cache->cache_type].apc_accessesDivActiveMemCy_ratio);
            sqlite3_bind_double(stmt, c++, lpm[cpu][cache->cache_type].lpmr_activeMemCyDivIdealCy_ratio);
            sqlite3_bind_double(stmt, c++, lpm[cpu][cache->cache_type].camat_activeMemCyDivAccesses_ratio);
            // totals
