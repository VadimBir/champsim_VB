        ...
        for (int i=0; i<NUM_CPUS; i++) {
            ...
            if (show_heartbeat && (ooo_cpu[i].num_retired >= ooo_cpu[i].next_print_instruction)) {
                ...
                cout << " " << STR_IPC_NOW << FIXED_FLOAT2(heartbeat_ipc)
                     ...
                     << " " << STR_BYPASS 
                     << setw(3) << FIXED_FLOAT2(ooo_cpu[i].L1D.sim_access[i][0] ? ooo_cpu[i].L1D.total_ByP_cnt / (double) ooo_cpu[i].L1D.sim_access[i][0] : 0) << " "
                     << setw(3) << FIXED_FLOAT2(ooo_cpu[i].L2C.sim_access[i][0] ? ooo_cpu[i].L2C.total_ByP_cnt / (double)  ooo_cpu[i].L2C.sim_access[i][0] : 0) << " "
                     << setw(3) << FIXED_FLOAT2(uncore.LLC.sim_access[i][0] ? uncore.LLC.total_ByP_cnt / (double) uncore.LLC.sim_access[i][0] : 0) << " "
                     << left 
                     << " " STR_APC
