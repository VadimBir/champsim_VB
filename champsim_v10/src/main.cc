        ...
        for (int i=0; i<NUM_CPUS; i++) {
            ...
            if (show_heartbeat && (ooo_cpu[i].num_retired >= ooo_cpu[i].next_print_instruction)) {
                ...
                cout << " " << STR_IPC_NOW << FIXED_FLOAT2(heartbeat_ipc)
                     ...
                     << "Ⓜ" << setw(6) << FIXED_FLOAT2(lpm[i][DRAM_type].apc_accessesDivActiveMemCy_ratio)
                     << left
                     << " " STR_LPM
                     << "⚙" << setw(5) << FIXED_FLOAT2(get_LPMR_global_std(i))
                     << "①" << setw(6) << FIXED_FLOAT2(lpm[i][L1D_type].lpmr_activeMemCyDivIdealCy_ratio)
                     << "②" << setw(6) << FIXED_FLOAT2(lpm[i][L2C_type].lpmr_activeMemCyDivIdealCy_ratio)
                     << "③" << setw(6) << FIXED_FLOAT2(lpm[i][LLC_type].lpmr_activeMemCyDivIdealCy_ratio)
